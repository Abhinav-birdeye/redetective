...
async function batchProcess({ cursor, standAloneClient, localClient, updateCursor, updateKeysMigrated }: BatchProcessOptions) {
    ...
    const { data: pipelineResult, error: pipelineError } = await tryCatch(readValuePipeline.exec());
    if (pipelineError) {
    ...
    const writePromises = oldKeysWithValue.map((key) => localClient.setex(key.key, 60 * 60 * 24, key?.value as string));
...
export async function migrateToLocal() {
    ...
    do {
        await batchProcess({ cursor, standAloneClient, localClient, updateCursor: (value: number) => { cursor = value; }, updateKeysMigrated: (value: number) => { keysMigrated += value; } });
        runs++;
